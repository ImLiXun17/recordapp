...
<body>
    ...
    $query_01 = "SELECT * FROM transaction ";
    ...
    if(!isset($_GET['page'])){
        ...
        //create query
        if (strlen($search) > 0 ){
            $query = "SELECT transaction.datelog, 
                             transaction.documentcode, 
                             transaction.action, office.name as office_name,
                             transaction.remarks, 
                             CONCAT(employee.lastname, ', ', employee.firstname) as employee_fullname

           FROM employee, 
                office,
                transaction 
           
           WHERE transaction.employee_id = employee.id and 
                 transaction.office_id = office.id and 
                 transaction.documentcode = $search ORDER BY transaction.documentcode, transaction.datelog
                ...
                <div class="container-fluid">
                    ...
                    <?php 
                                for($page=1; $page <= $number_of_page; $page++){
                                    echo '<a href="transaction.php?page='.$page.'" style="margin: right 3px">'. $page. ' </a>';
                                }
                            ?>
                </div>
            </div>
